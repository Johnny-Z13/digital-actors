...
# Import safe condition system
from scene_conditions import Condition, parse_condition_string
...
class SceneControl:
    ...
    visible_in_phases: list[int] | None = None  # None means always visible
    max_presses: int | None = None  # None means unlimited
    cooldown_seconds: float | None = None  # None means no cooldown
...
class SuccessCriterion:
    ...
    Attributes:
        ...
        condition: Either a condition string (legacy, parsed safely) or a Condition function
    ...
    id: str
    description: str
    condition: str | Condition  # String (parsed safely) or Condition function
    message: str
    required: bool = True
...
class VoiceEffect:
    ...
    mid_boost_freq: float | None = None
    mid_boost_gain: float | None = None
    mid_boost_q: float | None = None
...
class AudioAssets:
    ...
    volume_levels: dict[str, float] = field(
        default_factory=lambda: {"music": 0.5, "sfx": 0.8, "voice": 1.0}
    )
...
class SceneArtAssets:
    ...
    custom_scene_file: str | None = None
...
class SceneConstants:
    ...
    interruption_oxygen_penalty: int | None = None
    interruption_trust_penalty: int | None = None
    rapid_action_oxygen_penalty: int | None = None
    rapid_action_trust_penalty: int | None = None

    # Crisis event values
    crisis_oxygen_penalty: int | None = None
    crisis_trust_penalty: int | None = None
    help_oxygen_bonus: int | None = None
    help_trust_bonus: int | None = None

    # Difficulty adjustments
    easy_oxygen_bonus: int | None = None
    hard_oxygen_penalty: int | None = None

    # Thresholds
    critical_level: int | None = None  # e.g., critical oxygen level
    max_incorrect_actions: int | None = None

    # Director settings
    disable_events: bool = False  # Disable random events (e.g., phone scene)
    director_cooldown_override: int | None = None
...
class Scene:
    ...
    def evaluate_condition(self, condition: str | Condition, state: dict[str, Any]) -> bool:
        """
        Safely evaluate a condition with the current state.

        SECURITY: Uses safe condition parsing/evaluation - NO eval() of arbitrary code.

        Args:
            condition: Either a condition string (parsed with AST) or a Condition function
        ...
        try:
            if isinstance(condition, str):
                # Parse string condition safely (no eval)
                condition_fn = parse_condition_string(condition)
                return condition_fn(state)
            else:
                # Already a Condition function, evaluate directly
    ...
    def check_success(self, state: dict[str, Any]) -> SuccessCriterion | None:
    ...
    def check_failure(self, state: dict[str, Any]) -> FailureCriterion | None:
