...
from director_rules import RuleAction, RuleDecision, get_director_rules
...
class WorldDirector:
    ...
    async def evaluate_situation(
        ...
        scene_constants = SCENE_SPECIFIC_CONSTANTS.get(
            scene_id, SCENE_SPECIFIC_CONSTANTS["default"]
        )
        ...
        player_failed_attempts = (
            player_memory.scene_attempts.get(scene_id, 0) if player_memory else 0
        )
        ...
        if rule_decision.action != RuleAction.CONSULT_LLM:
            logger.info(
                "[Director] Fast rule matched: %s - %s",
                rule_decision.action.value,
        ...
        context = self._build_director_context(
            scene_id, scene_state, dialogue_history, player_memory, character_id, last_action
        )
...
{{{{
    ...
    def _build_director_context(
        ...
        pattern_str = (
            "Struggling - failed multiple times"
            if scene_attempts >= 2
            else "First attempt or doing well"
        )
        ...
        if "submarine" in scene_id.lower():
            ...
            scene_type_desc = (
                "This is a CONVERSATION scene focused on dialogue and character interaction."
            )
...
"""
    ...
    def _parse_director_response(self, response: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"action": "continue", "details": {}}
    ...
    def generate_dynamic_event(
        ...
        scene_key = (
            ...
            if "court" in scene_id.lower()
            else "default"
        )
        scene_constants = SCENE_SPECIFIC_CONSTANTS.get(
            scene_key, SCENE_SPECIFIC_CONSTANTS["default"]
        )
    ...
    def should_force_game_over(
        ...
        if (
            "jury_sympathy" in scene_state
            and scene_state["jury_sympathy"] <= scene_constants["critical_level"]
        ):
            return "failure"
        ...
        if "trust" in scene_state and scene_state["trust"] < scene_constants["low_threshold"]:
            ...
            if (
                "oxygen" in scene_state
                and scene_state["oxygen"] < scene_constants["critical_level"]
            ):
                return "failure"

        if (
            "judge_trust" in scene_state
            and scene_state["judge_trust"] < scene_constants["low_threshold"]
        ):
            return "failure"
    ...
    def generate_npc_behavior_adjustment(
        ...
        resource_var = (
            "oxygen"
            if "oxygen" in current_state
            else "jury_sympathy"
            if "jury_sympathy" in current_state
            else "time_remaining"
            if "time_remaining" in current_state
            else None
        )
    ...
    def generate_hint(
        self, scene_id: str, hint_type: str, hint_content: str, character_id: str
    ) -> str:
        """
    ...
    def get_difficulty_adjustment(
        ...
        success_rate = player_memory.total_successes / max(1, player_memory.total_scenes_played)
        ...
        if (
            ...
            if (
                "oxygen" in scene_constants["resource_name"]
                if scene_constants["resource_name"]
                else False
            ):
